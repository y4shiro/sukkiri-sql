...
# 第 8 章 複数テーブルの結合
# 65. 次の口座について、これまでの取引の記録を取引テーブルから抽出する。抽出する項目は口座番号、日付、取引事由名、取引金額とする
# 口座番号ごとに取引番号順で表示し、取引事由名については取引事由テーブルから日本語名を取得する
# 取引金額には、取引に応じて入金額か出金額のいずれか適切な方を表示すること
# 口座番号: 0311240 / 1234161 / 2750902
SELECT 口座番号,
       日付, 
       (SELECT 取引事由名 
          FROM 取引事由
         WHERE 取引.取引事由ID = 取引事由.取引事由ID) AS 取引事由名,
       CASE
...
# 66. 次の口座について、口座情報(口座番号/名義/残高)とこれまでの取引情報(日付/入金額/出金額)を一覧として抽出する
# 一覧は、取引の古い順に表示すること
# 口座番号: 0887132
...
# 67. 2020/03/01 に取引のあった口座番号の一覧を取得する。一覧には、口座テーブルより名義と残高も表示すること
# ただし、解約された口座については考慮しなくていい
...
# 68. 問題 67 では、既に解約された口座については、当該の日付に取引があるものの抽出されなかった
# これを改良して解約口座ももれなく抽出出来るようにする
# なお、解約口座に関しては名義に '解約済み'、残高に 0 を表示する
...
# 69. 取引テーブルのデータを抽出する。取引事由は '取引事由ID: 取引事由名' の形式で表示し、これまで発生しなかった取引事由についても併せて記載すること
SELECT T.取引番号, CONCAT(T.取引事由ID, ':', J.取引事由名) AS 取引事由, T.日付, T.口座番号, T.入金額, T.出金額
  FROM 取引 AS T
  RIGHT JOIN 取引事由 AS J
    ON J.取引事由ID = T.取引事由ID;

# 70. 取引テーブルと取引事由テーブルから、取引事由の一覧を抽出する。一覧には、取引事由 ID 取引事由名を記載する。
# なお、取引事由テーブルに存在しない事由で取引されている可能性、および取引の実績の無い事由が存在する可能性を考慮する
# FULL JOIN が使える場合
...
# 71. 問題 66 について、取引事由名についても一覧に表示するよう、SQL 文を変更する
# 取引事由名は取引情報(日付、取引事由名、入金額、出金額) に表示する
...
# 72. 現在の残高が 500 万円以上の口座について、2022 年以降に 1 回の取引で 100 万円以上の金額が入出金された実績を抽出する
# 抽出項目は、口座番号 / 名義 / 残高 / 取引の日付 / 取引事由 ID / 入金額 / 出金額 とし、副問合せは用いない事とする
...
# 73. 問題 72 で作成した SQL 文について、結合相手に副問合せを利用するよう SQL を変更する
# 口座テーブルを副問合せにした場合
SELECT K.口座番号, K.名義, K.残高, T.日付, T.取引事由ID, T.入金額, T.出金額
  FROM 取引 AS T
...
SELECT K.口座番号, K.名義, K.残高, T.日付, T.取引事由ID, T.入金額, T.出金額
  ...
  JOIN (SELECT 口座番号, 日付, 取引事由ID, 入金額, 出金額
    ...
    ON K.口座番号 = T.口座番号
  WHERE K.残高 >= 5000000;
# 74. 取引テーブルから、同一の口座で同じ日に 3 回以上取引された実績のある口座番号とその回数を抽出する
...
# 名義 / 口座番号 / 種別 / 残高 / 更新
# 一覧は名義の五十音順、口座番号の小さい順に並べること
# 自分の解答、集計関数と結合を用いた場合
SELECT K1.名義, K1.口座番号, K1.種別, K1.残高, K1.更新日
  FROM 口座 AS K1
 INNER JOIN (SELECT 名義, COUNT(名義) AS 口座数
               FROM 口座
              GROUP BY 名義
             HAVING COUNT(名義) > 1) K2
    ON K1.名義 = K2.名義
