...
-- 第 8 章 複数テーブルの結合
-- 62. 既にクリアしたイベントについて、次の形式の一覧を抽出する
...
-- 63. イベントテーブルから、タイプ '強制' のイベントについて、イベント番号 / イベント名称 / クリア区分を抽出する
-- 未着手のイベントは考慮しなくていい
...
-- 64. 問題 63 では、着手していないイベントは抽出されなかった。今回は未着手のイベントも抽出できるよう SQL 文を変更する
-- クリアしていないイベントについては、クリア区分に '未クリア' と表示する
...
-- コード種別 / コード値 / コード名称
-- このテーブルを使って、現在のパーティーに参加しているキャラクターの一覧を適切な列を用いて別名で表示する、ID 順にソートする
...
-- 66. パーティーテーブルから、現在のパーティーに参加しているキャラクターの一覧を次の形式で抽出する
-- 職業はコードテーブルより日本語で表示する。また、現在のパーティーに居ない職業についても漏れなく一覧に記載し、
-- 名称の項目に '仲間になっていない!' と表示すること
-- ID / なまえ / 職業
select P.id,
       coalesce (P.名称, '仲間になっていない!' )as 名前,
       S.コード名称 as 職業
  from パーティー as P
...
-- 67. 経験イベントテーブルから、参加済みイベントのクリア結果一覧を次の形式で抽出する
-- イベント番号 / クリア区分 / クリア結果
-- クリア結果は 'コード値 : コード名称' のように表示し、クリア未済のイベントも記載されるよう考慮する
-- また、まだ記録していないクリア結果の全てのコード値についても一覧に記載する
select E.イベント番号, E.クリア区分, C.コード値 || ':' || C.コード名称
  from 経験イベント as E
...
-- 70. 他のイベントの前提となっているイベントについて、次の形式の一覧を抽出する。一覧はイベント番号順とする
